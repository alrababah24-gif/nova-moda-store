...
export function ProductDetailClient({
  ...
  return (
    <>
      {/* ✅ هذا السطر هو اللي بصلح مشكلة ViewContent ومطابقة الكتالوج 0% */}
      <FacebookViewContent
            ...
            <div className="mt-3 flex gap-2 overflow-auto">
              {images.map((img, i) => (
                <button
                  ...
                  className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-xl border ${currentImg === i ? "border-black" : "border-transparent"}`}
                >
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            {product.originalPrice && (
              <span className="text-sm line-through text-[var(--muted)]">{formatPrice(product.originalPrice)}</span>
            )}
          </div>
                ...
                {product.sizes.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => setSize(s)}
                      disabled={stock === 0}
                      className={`min-h-10 rounded-full border px-4 text-sm ${size === s ? "border-black bg-black text-white" : "border-[var(--line)]"} ${stock === 0 ? "opacity-40" : ""}`}
                    >
                      {s}
                    </button>
                  );
          ...
          {product.colors && product.colors.length > 0 && (
            <div className="mt-5">
              <div className="mb-2 text-sm font-medium">اللون: {color}</div>
              <div className="flex flex-wrap gap-2">
                {product.colors.map((c) => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
                    className={`h-9 rounded-full border px-4 text-sm ${color === c ? "border-black bg-black text-white" : "border-[var(--line)]"}`}
                  >
                    {c}
                  </button>
                ))}
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <button
              ...
              className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-[#3D2B24] px-5 text-sm font-medium text-white disabled:opacity-50"
            >
              <ShoppingBag size={18} />
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-full border border-[var(--line)] text-sm font-medium"
          >
