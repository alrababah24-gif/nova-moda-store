  ...
  return <div className="space-y-5">
    ...
    {editorOpen && <section ref={editorRef} className="product-editor-panel rounded-[26px] border border-[#DCCABD] bg-white p-5 soft-shadow sm:p-6">
      ...
      <div className="mt-5 rounded-2xl border border-[#EEE1D7] bg-[#FFFCF9] p-4">
        <div className="mb-3 flex items-center justify-between gap-3"><div><strong className="text-xs">صور المنتج ({draft.images.length}/8)</strong><p className="mt-1 text-[10px] text-[#8C7A72]">الصورة الأولى هي الغلاف. أي أبعاد ترفعيها يتم احتواؤها داخل الإطار بدون تكبير أو قص مزعج.</p></div><label className="flex cursor-pointer items-center gap-2 rounded-full border border-[#DDCBBE] bg-white px-4 py-2 text-xs font-bold"><ImagePlus size={14}/>{uploading?"جاري الرفع...":"اختيار الصور"}<input type="file" accept="image/*" multiple hidden onChange={upload} disabled={uploading||draft.images.length>=8}/></label></div>
        <div className="flex flex-wrap gap-3">{draft.images.map((url,index)=><div key={`${url}-${index}`} className="relative h-32 w-24 overflow-hidden rounded-xl border border-[#E6D8CE] bg-[#FDF6F0]"><Image src={url} alt={`صورة ${index+1}`} fill className="object-contain p-1" sizes="96px"/><span className="absolute bottom-1 right-1 rounded-full bg-black/65 px-2 py-0.5 text-[8px] font-bold text-white">{index===0?"غلاف":index+1}</span><button onClick={()=>setDraft({...draft,images:draft.images.filter((_,imageIndex)=>imageIndex!==index)})} className="absolute left-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-white/90 text-red-600"><X size={12}/></button></div>)}</div>
      </div>
