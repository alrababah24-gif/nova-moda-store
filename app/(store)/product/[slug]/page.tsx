    ...
    <>
      ...
      <section className="py-7 sm:py-11">
        <div className="container-shell">
          <div className="mb-6 flex items-center gap-2 text-xs text-[#8C7A72]">
            ...
            {currentProduct.brand && (
              <>
                <span>/</span>
                <Link href={`/brands/${currentProduct.brand.slug}`}>{currentProduct.brand.name}</Link>
              </>
            )}
            <span>/</span>
            <span className="truncate">{currentProduct.name}</span>
          </div>
          <ProductDetailClient product={currentProduct} settings={settings} />
        </div>
      </section>
      {related.length > 0 && (
        <section className="border-t border-[#F0E6DC] py-14">
          <div className="container-shell">
            <div className="mb-6 flex items-end justify-between">
              <div>
                <p className="text-xs font-extrabold text-[#A27E6C]">YOU MAY ALSO LIKE</p>
                <h2 className="mt-2 text-2xl font-extrabold">قد يعجبك أيضاً</h2>
              </div>
              <span className="text-xs text-[#8C7A72]">ابتداءً من {formatPrice(Math.min(...related.map((p) => p.price)))}</span>
            </div>
            <ProductGrid products={related} categories={categories} showFilters={false} />
          </div>
        </section>
      )}
    </>
